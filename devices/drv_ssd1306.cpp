 ...
 */
SSD1306::SSD1306(DriverInOutBase *port_handle, uint8_t address_8bits)
{
 ...
 * @return Status_t
 */
Status_t SSD1306::writeAsync(const uint8_t *buffer, uint32_t size, uint8_t key, InOutStreamCallback_t func, void *arg)
{
  (void) key;
  if(!m_is_initialized) initialize();
...
Status_t SSD1306::writePixels(uint8_t *buffer, uint32_t size)
{
  ...
  if(!m_driver->getWriteStatus().success) { return STATUS_DRV_ERR_BUSY;}
  m_internal_buffer[0] = SD1306_DATA;
  memcpy(&m_internal_buffer[1], buffer, size);
  return m_driver->write(m_internal_buffer, size + 1, UINT32_MAX);
}
