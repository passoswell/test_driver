...
AP_MAIN()
{
  ...
  std::printf("Maximum queue capacity: %u\r\n", my_queue.getMaxSize());
  std::printf("Elements on queue: %u\r\n", my_queue.getActualSize());

  for (uint16_t i = 0; i < 6; i++)
  {
    if (my_queue.put(i, 5000))
    {
      std::printf("Success enqueuing %u\r\n", i);
      std::printf("\r\nElements on queue: %u\r\n", my_queue.getActualSize());
    }
    else
    {
      std::printf("Failed enqueuing %u\r\n", i);
    }
  }
  ...
  for (uint16_t i = 0; i < 6; i++)
  {
    ...
    {
      std::printf("Success dequeuing %u\r\n", item);
      std::printf("\r\nElements on queue: %u\r\n", my_queue.getActualSize());
    }
    else
    {
      std::printf("Failed dequeuing %u\r\n", i);
    }
  }
