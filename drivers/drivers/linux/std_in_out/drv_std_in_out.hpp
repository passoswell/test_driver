...
public:
  ...
  Status_t readAsync(uint8_t *buffer, uint32_t size, uint32_t key = INOUTSTREAM_MASTER_KEY, InOutStreamCallback_t func = nullptr, void *arg = nullptr);
  bool isReadAsyncDone();

  Status_t writeAsync(uint8_t *buffer, uint32_t size, uint32_t key = INOUTSTREAM_MASTER_KEY, InOutStreamCallback_t func = nullptr, void *arg = nullptr);
