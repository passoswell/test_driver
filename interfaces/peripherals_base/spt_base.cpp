...
#include "spt_base.hpp"

/**
 * @brief Constructor
 */
SptBase::SptBase(SoftwareTimerCountUnit_t time_unit)
{
  m_start_time = 0;
  m_duration = 0;
  m_is_running = false;
 ...
 */
void SptBase::start(uint32_t duration, SoftwareTimerMode_t mode)
{
 ...
 * @return uint32_t
 */
uint32_t SptBase::getElapsedTime()
{
  sft_time_us_t now_time = getTimeSincePowerOnUs();
  sft_time_us_t difference = now_time - m_start_time;
  uint32_t time;
 ...
 * @param duration
 */
void SptBase::delay(uint32_t duration)
{
